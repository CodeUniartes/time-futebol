...
class SavedOrdersWindow(ctk.CTkToplevel):
    def __init__(self, master, order_service, on_load):
        ...
        ctk.CTkButton(buttons, text="Carregar pedido", command=self.load_selected).grid(row=0, column=1, padx=8)
        ctk.CTkButton(buttons, text="Fechar", fg_color="#64748b", command=self.destroy).grid(row=0, column=2)
        self.refresh()
